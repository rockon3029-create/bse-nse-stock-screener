...
data class MarketSummary(
    val date: String,
    val bestSector: String,
    val bestSectorGain: String,
    val worstSector: String,
    val worstSectorGain: String,
    val marketSummary: String,
    val newsSourceTitle: String,
    val newsSourceLink: String,
    val forecastSector: String,
    val forecastSectorReason: String,
    val forecastShareSymbol: String,
    val forecastShareName: String,
    val forecastSharePrice: String,
    val forecastShareGain: String,
    val forecastShareReason: String,
    val forecastReference: String
)
...
object StockRepository {
        private const val GITHUB_SUMMARY_URL =
        "https://raw.githubusercontent.com/rockon3029-create/bse-nse-stock-screener/main/market_summary.json"

    suspend fun fetchMarketSummary(): MarketSummary? = withContext(Dispatchers.IO) {
        try {
            ...
            client.newCall(request).execute().use { res ->
                ...
                MarketSummary(
                    ...
                    bestSector = json.optString("bestSector", "Renewable Energy"),
                    bestSectorGain = json.optString("bestSectorGain", "+3.2%"),
                    worstSector = json.optString("worstSector", "Telecom"),
                    worstSectorGain = json.optString("worstSectorGain", "-1.1%"),
                    marketSummary = json.optString("marketSummary", "Indices experienced healthy sector rotation."),
                    newsSourceTitle = json.optString("newsSourceTitle", "Market News"),
                    newsSourceLink = json.optString("newsSourceLink", "https://www.moneycontrol.com"),
                    forecastSector = json.optString("forecastSector", "Renewables & Power"),
                    forecastSectorReason = json.optString("forecastSectorReason", "Strong momentum and order-inflows."),
                    forecastShareSymbol = json.optString("forecastShareSymbol", "SUZLON"),
                    forecastShareName = json.optString("forecastShareName", "Suzlon Energy"),
                    forecastSharePrice = json.optString("forecastSharePrice", "₹48.0"),
                    forecastShareGain = json.optString("forecastShareGain", "+4.2%"),
                    forecastShareReason = json.optString("forecastShareReason", "High relative strength and volume expansion."),
                    forecastReference = json.optString("forecastReference", "NSE Sector Analytics")
                )
            }
        } catch (_: Exception) {
            null
